...
from PyQt6.QtWidgets import (
    ...
    QPlainTextEdit, QProgressBar, QSplitter, QFrame,
...
class SettingsDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        advanced_content_layout.addWidget(QLabel("Folder name for quotes within customer directory (e.g., 'Quotes')"))
        self.quote_folder_edit = QLineEdit(
            self.settings.get('quote_folder_path', 'Quotes')
        )
...
class JobDocs(QMainWindow):
    ...
    DEFAULT_SETTINGS = {
        ...
        'quote_folder_path': 'Quotes',  # Path for quotes relative to customer directory
    ...
    def browse_quote_files(self):
        files, _ = QFileDialog.getOpenFileNames(self, "Select Files")
        if files:
            self.add_quote_files(files)
    ...
    def create_quote(self):
        ...
        if not customer or not quote_numbers_text or not description:
            QMessageBox.warning(self, "Error", "Customer, Quote #, and Description are required")
            return
        ...
        # Parse quote numbers (same logic as job numbers)
        quote_numbers = self.parse_job_numbers(quote_numbers_text)
        if not quote_numbers:
            QMessageBox.warning(self, "Error", "Invalid quote number format")
            return
        ...
        if created > 0:
            QMessageBox.information(self, "Success", f"Created {created}/{len(quote_numbers)} quote(s)")
        else:
            QMessageBox.warning(self, "Error", "Failed to create quotes")
    ...
    def create_single_quote(self, customer: str, quote_number: str, description: str,
        ...
        try:
            ...
            # Build quote folder name (same format as jobs)
            if drawings:
                quote_dir_name = f"{quote_number}_{description}_{'-'.join(drawings)}"
            else:
                quote_dir_name = f"{quote_number}_{description}"

            quote_dir_name = re.sub(r'[<>:"/\\|?*]', '_', quote_dir_name)

            # Build quote path (customer_files/<customer>/<quotes_folder>/<quote_dir_name>)
            quote_folder_path = self.settings.get('quote_folder_path', 'Quotes')
            ...
            for file_path in files:
                ...
                if self.is_blueprint_file(file_name):
                    ...
                    if not quote_dest.exists():
                        self.create_link(bp_dest, quote_dest)
                else:
            ...
            QMessageBox.warning(self, "Error", f"Error creating quote {quote_number}: {e}")
            return False
    ...
    def convert_current_quote_to_job(self):
        ...
        if not customer or not quote_number or not description:
            QMessageBox.warning(self, "Error", "Please fill in Customer, Quote #, and Description before converting")
            return
        ...
        self.job_number_edit.setText(quote_number)
        self.description_edit.setText(description)
        self.drawings_edit.setText(drawings)

        # Copy ITAR setting
        self.itar_check.setChecked(self.quote_itar_check.isChecked())

        # Copy files
        self.job_files = self.quote_files.copy()
        self.job_files_list.clear()
        for file in self.job_files:
            self.job_files_list.addItem(os.path.basename(file))

        QMessageBox.information(self, "Quote Converted", f"Quote {quote_number} data copied to Create Job tab.\n\nYou can now create the job from this quote.")

    # ==================== Search & Copy Functions ====================

    def search_for_job_copy(self):
        """Search for existing jobs/quotes to copy info from (for Create Job tab)"""
        search_term = self.job_search_input.text().strip().lower()
        self.job_search_results.clear()

        if len(search_term) < 2:
            return

        # Search in both customer files and quote directories
        cf_dir = self.settings.get('customer_files_dir', '')
        itar_cf_dir = self.settings.get('itar_customer_files_dir', '')

        results = []

        for base_dir, prefix in [(cf_dir, ''), (itar_cf_dir, '[ITAR] ')]:
            if not base_dir or not os.path.exists(base_dir):
                continue

            # Recursively search for job/quote folders
            for root, dirs, files in os.walk(base_dir):
                for dir_name in dirs:
                    dir_lower = dir_name.lower()
                    if search_term in dir_lower:
                        full_path = os.path.join(root, dir_name)
                        rel_path = os.path.relpath(full_path, base_dir)
                        results.append((prefix + rel_path, full_path))

        # Add to list widget
        for display_name, full_path in sorted(results)[:50]:  # Limit to 50 results
            item = self.job_search_results.addItem(display_name)
            # Store full path in item data
            self.job_search_results.item(self.job_search_results.count() - 1).setData(Qt.ItemDataRole.UserRole, full_path)

    def copy_job_to_form(self):
        """Copy selected job/quote info to Create Job form"""
        current_item = self.job_search_results.currentItem()
        if not current_item:
            return
        ...
        # Parse folder name (format: job#_description_drawings or job#_description)
        ...
        if len(parts) >= 2:
            ...
            # Get customer name (parent folder)
            customer = os.path.basename(os.path.dirname(folder_path))
    ...
    def link_files_from_job(self):
        """Link files from selected job/quote to current job being created"""
        current_item = self.job_search_results.currentItem()
        if not current_item:
            QMessageBox.warning(self, "No Selection", "Please select a job/quote from the search results")
            return
        ...
        if not files_to_add:
            QMessageBox.information(self, "No Files", "No files found in selected job/quote folder")
            return

        # Add files to job files list
        for file in files_to_add:
            if file not in self.job_files:
                self.job_files.append(file)
                self.job_files_list.addItem(os.path.basename(file))

        QMessageBox.information(self, "Files Added", f"Added {len(files_to_add)} files from selected job/quote")

    def search_for_quote_copy(self):
        """Search for existing jobs/quotes to copy info from (for Create Quote tab)"""
        search_term = self.quote_search_input.text().strip().lower()
        self.quote_search_results.clear()
        ...
        # Add to list widget
        for display_name, full_path in sorted(results)[:50]:  # Limit to 50 results
            item = self.quote_search_results.addItem(display_name)
            # Store full path in item data
            self.quote_search_results.item(self.quote_search_results.count() - 1).setData(Qt.ItemDataRole.UserRole, full_path)

    def copy_quote_to_form(self):
        """Copy selected job/quote info to Create Quote form"""
        current_item = self.quote_search_results.currentItem()
        if not current_item:
            return
        ...
        if len(parts) >= 2:
            ...
            if customer.lower() == 'quotes' or customer.lower() == self.settings.get('quote_folder_path', 'Quotes').lower():
    ...
    def link_files_from_quote(self):
        """Link files from selected job/quote to current quote being created"""
        current_item = self.quote_search_results.currentItem()
        if not current_item:
            QMessageBox.warning(self, "No Selection", "Please select a job/quote from the search results")
            return
    ...
    def log_message(self, message: str):
        """Log messages - currently unused since log widget was removed"""
        # Could add status bar messages here if needed
        pass
